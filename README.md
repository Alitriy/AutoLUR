# AutoLUR - 自动化 Land Use Regression (LUR) 建模系统

AutoLUR 是一个面向环境暴露分析与空间建模的自动化机器学习框架。它结合了空间交叉验证、多层特征工程、大规模模型集成以及创新的**层级 SHAP 特征依赖图 (Hierarchical SHAP Feature Dependence)** 分析技术，旨在为研究人员提供端到端、一键式、出版级的 LUR 建模解决方案。
...
- [⚙️ 项目核心原理](#-项目核心原理)
  ...
  - [层级 SHAP 依赖图分析 (新增)](#3-层级-shap-依赖图分析-新增)
- [📂 项目架构与输出结构](#-项目架构与输出结构)
...
为了深度挖掘特征与特征之间的复杂交互作用，AutoLUR 创新性地引入了 **层级 SHAP (Hierarchical SHAP)** 解析架构。传统的交互作用计算受限于算法支持度，而我们的分层架构保证了 100% 的子模型覆盖率：
